...
This package reproduces all empirical results, simulations, and figures in the paper.
...
├── README.md                          ← this file
│
├── DATA (pre-built, checked-in)
│   ├── panel_data.csv                 ← 856-obs monthly US panel (1954–2025)
│   ├── cross_country_panel_data.csv   ← 2506-obs cross-country panel (6 countries)
│   └── scf_alpha_data.csv             ← superseded; SCF data now in panel_data.csv
│
├── ANALYSIS SCRIPTS (run directly, no downloads needed)
...
│   └── paper_figures.py               ← all 12 paper figures (PDF + PNG)
│
└── DATA PIPELINE (requires raw external files — see below)
    └── data_pipeline.py               ← builds panel_data.csv from scratch
```
...
# All 12 paper figures → figures/ directory
...
non-NaN rows are used by `robustness_analysis.py` Section 7 and `paper_figures.py`
Figure 1. `scf_alpha_data.csv` is superseded; this is the canonical source.

| Column | Description |
|---|---|
| `scf_fof_narrow` | FoF-comparable narrow equity share computed from SCF microdata (directly-held equities + mutual funds only; cross-validates the `alpha` column) |
...
### `scf_alpha_data.csv` ← superseded

This file is retained for backward compatibility. The same data now lives in the
`scf_*` columns of `panel_data.csv` and should be read from there.

---

## Key Parameters (Headline Specification)

These values are hard-coded in the simulation and figure scripts; they come from
...
Output: `panel_data.csv` written to the same directory as the script. The SCF
columns (`scf_*`) are not rebuilt by this script — they come from `scf_alpha_data.csv`
and should be merged back in after rebuilding using the first cell of the data audit
notebook or the merge snippet in the README history.
...
> Dixon, O. (2026). *The Effective Rate: Household Portfolio Returns and
